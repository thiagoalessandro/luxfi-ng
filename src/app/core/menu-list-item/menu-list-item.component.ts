...
export class MenuListItemComponent implements OnInit {
  ...
  ngOnInit() {     
    NavService.currentUrl.subscribe(url => {
      var pathSplit = NavService.getSplitPath(url);
      if (pathSplit.length == 2) {
          this.expanded = this.menu.listSubMenu.filter(submenu => submenu.funcionalidade.rota === pathSplit[0]).length > 0;
  ...
  public onSubMenuSelected(menu: SegMenuModel, subMenu: SegSubMenuModel) {
    this.router.navigate([`/${subMenu.funcionalidade.rota}`]);
  }
  ...
  public subMenuActive(menu: SegMenuModel, subMenu: SegSubMenuModel): boolean{
    ...
    NavService.currentUrl.subscribe(url => {
      var pathSplit = NavService.getSplitPath(url);      
        result = (subMenu.funcionalidade.rota == pathSplit[0]);
    })
    return result;
  }
